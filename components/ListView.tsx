...
const TaskRow = memo(({ task, isSelected, locale, onSelectTask, t }: TaskRowProps) => {
  ...
  return (
    ...
    >
      <td className="py-2.5 px-3 text-xs font-mono text-slate-400">{task.wbs || '-'}</td>
      <td className="py-2.5 px-3">
         <div className="flex flex-col">
            <span className={`text-sm font-medium flex items-center gap-1.5 ${task.isMilestone ? 'text-amber-700' : 'text-slate-700'}`}>
               {task.isMilestone && (
                 <svg className="w-4 h-4 text-amber-500" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true"><path d="M6 2a1 1 0 00-1 1v1H4a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V6a2 2 0 00-2-2h-1V3a1 1 0 10-2 0v1H7V3a1 1 0 00-1-1zm0 5a1 1 0 000 2h8a1 1 0 100-2H6z" /></svg>
               )}
               {task.title}
            </span>
            {task.description && (
      ...
      <td className="py-2.5 px-3">
         <span className={`text-xs px-2 py-0.5 rounded-full border font-semibold ${priorityColors[task.priority]}`}>
            {getPriorityLabel(task.priority, t)}
         </span>
      </td>
      <td className="py-2.5 px-3">
        <span className={`flex items-center gap-1.5 text-xs font-medium ${statusColors[task.status]}`}>
            <span className={`w-1.5 h-1.5 rounded-full ${task.status === TaskStatus.TODO ? 'bg-slate-400' : task.status === TaskStatus.IN_PROGRESS ? 'bg-indigo-500' : 'bg-emerald-500'}`} aria-hidden="true" />
      ...
      <td className="py-2.5 px-3">
         <div className="flex items-center gap-2 w-full max-w-[120px]">
            <div className="flex-1 bg-slate-100 h-1.5 rounded-full overflow-hidden">
               <div className={`h-full rounded-full ${task.completion === 100 ? 'bg-emerald-500' : 'bg-indigo-500'}`} style={{ width: `${task.completion || 0}%`}} aria-label={`${task.completion || 0}% complete`} />
            </div>
            <span className="text-xs w-6 text-right text-slate-500">{task.completion || 0}%</span>
         </div>
      </td>
       ...
       <div className="overflow-auto custom-scrollbar flex-1">
        <table className="w-full text-left border-collapse">
          <thead className="bg-slate-50/80 backdrop-blur-sm sticky top-0 z-10 border-b border-slate-200">
            <tr>
              <th className="py-2.5 px-3 text-xs font-bold text-slate-400 uppercase tracking-wider w-16">{t('list.header.wbs')}</th>
              <th className="py-2.5 px-3 text-xs font-bold text-slate-400 uppercase tracking-wider min-w-[200px]">{t('list.header.task_name')}</th>
              <th className="py-2.5 px-3 text-xs font-bold text-slate-400 uppercase tracking-wider w-32">{t('list.header.assignee')}</th>
              <th className="py-2.5 px-3 text-xs font-bold text-slate-400 uppercase tracking-wider w-24">{t('list.header.priority')}</th>
              <th className="py-2.5 px-3 text-xs font-bold text-slate-400 uppercase tracking-wider w-28">{t('list.header.status')}</th>
              <th className="py-2.5 px-3 text-xs font-bold text-slate-400 uppercase tracking-wider w-32">{t('list.header.progress')}</th>
              <th className="py-2.5 px-3 text-xs font-bold text-slate-400 uppercase tracking-wider w-28">{t('list.header.start')}</th>
              <th className="py-2.5 px-3 text-xs font-bold text-slate-400 uppercase tracking-wider w-28">{t('list.header.due')}</th>
            </tr>
          </thead>
               ...
               <tr>
                 <td colSpan={8} className="py-12 text-center">
                    <div className="flex flex-col items-center justify-center">
                       ...
                       <span className="text-sm text-slate-400 italic">{t('list.empty')}</span>
                    </div>
                 </td>
               </tr>
            ) : (
